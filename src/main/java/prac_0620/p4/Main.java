...
public class Main {
    public static void main(String[] args) {
        ...
        //메모리값이 s3은 4고 s4는 0이 된다. s3의 메모리 공간을 같이 가리키게 되기 때문. (리모콘을 받아온다) 가리키는 메모리공간이 중요하다는 것을 알 수 있다.
        //이렇게 되는 이유는 메모리공간을 자동으로 아끼려고 하기 때문
        //new String으로 추가해준 s2는 '재활용모드'가 꺼진 상태이기에 만약 s1이 "하하"였어도 재활용하지 못하고 s1에 연결될 놈을 새로 만든다.
        //마찬가지로 s3도 s2를 재활용하지 못한다. s2의 '재활용모드'가 꺼진 상태이기 때문에, 재활용을 하지도 되지도 못하는 것.
        //따라서 s2==s3은 false로 나온다.
        //만약 s4가 "하하하"였다면, s3의 "하하"를 재활용하지 않고 아예 새로 만든다는 점에도 유의.
        System.out.println("s2==s3 : " + (s2==s3));
        System.out.println("s3.equals(s4) : " + s3.equals(s4)); //이건 true
